...
%     Para.storage(1).chahour = [11,12,13];
    ...
    Para.storage(1).discurve(20) = 20;
    Para.storage(1).discurve(21) = 20;
    Para.storage(1).chacurve = zeros(1,24);
    Para.storage(1).chacurve(10) = 15;
    Para.storage(1).chacurve(11) = 15;
    Para.storage(1).chacurve(12) = 20;
    
    %% 这个表示PSD
    Para.storage(2).discurve = zeros(1,24);  % 这里设计一个charge curve和discharge curve
    %Para.storage(2).discurve(19) = 10;
    Para.storage(2).discurve(20) = 20;
    Para.storage(2).discurve(21) = 20;
    %Para.storage(2).discurve(22) = 10;
    
    Para.storage(2).chacurve = zeros(1,24);
    Para.storage(2).chacurve(13) = 20;
    Para.storage(2).chacurve(14) = 20;
   %  Para.storage(2).chacurve(13) = 25;
...
% 
    ...
    %         for w = 1:Num.S
                ...
                Para.demand(d).Utility(t,:) = 100  * ones(Num.B,1);
    %         end 
    ...
    for t = 1:Num.T
        Para.demand(Num.D).Utility(t,:) = 100 * [1;0.7;0.4;0.4];
    end
    ...
    Para.demand(Num.D).Bus = 4;
    Para.demand(Num.D).Pmax = Para.demand(4).Pmax * 0.05;
    Para.demand(Num.D).Pmin = Para.demand(4).Pmin;
    Para.demand(4).Pmax = Para.demand(4).Pmax * 0.95; %把最大的一个demand想办法调小
    for t = 1:Num.T
        Para.demand(Num.D).Utility(t,:) = 100  * ones(Num.B,1);
    end
   ...
   %% 生成Para_modify数据, 为什么要modify数据? 要方便进行market clearing
    ...
    for t = 1:Num.T
        ...
        for s = 1:Num.ESS
            ...
            Para_modify.ESS(t).Pchamax(s,:) = Para.storage(s).Pchamaxb *  Para.storage(s).chacurve(t) / Para.storage(s).Pchamax;
...
%             end
            Para_modify.ESS(t).Pdismax(s,:) = Para.storage(s).Pdismaxb *  Para.storage(s).discurve(t) / Para.storage(s).Pdismax;
...
%     newdata(Num.L,2) = 0; %截距
    ...
    for nnode = 1:Num.N
        noderenew_set = Para_modify.nodeinstrument(nnode).R + length(Para.genset);
        nodedemand_set = Para_modify.nodeinstrument(nnode).D;
        Para_modify.nodenet(:,nnode) = sum([Para.demand(nodedemand_set).Pmax],2) - sum([Para.generator(noderenew_set).Pmax],2);
    end 
...
%     for nnode = 1:Num.N
%         ratio = sum(Para_modify.nodenet(:,nnode))/sum(sum(Para_modify.nodenet));
%         Para_modify.node_costcurve(nnode).Pmax = Para_modify.costcurve.Pmax * ratio;
%         Para_modify.node_costcurve(nnode).Pmin = Para_modify.costcurve.Pmin * ratio;
%         Para_modify.node_costcurve(nnode).Plen = Para_modify.costcurve.Plen * ratio;
%         Para_modify.node_costcurve(nnode).x = Para_modify.costcurve.x;
%         Para_modify.node_costcurve(nnode).y = Para_modify.costcurve.y;
%     end
